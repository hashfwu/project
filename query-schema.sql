...
create table Persona (
  ci varchar(15) primary key, -- 1234567, sin expedido
  expedido varchar(50), -- LP (La Paz), SCZ (Santa Cruz), CBBA (Cochabamba), OR (Oruro), PT (Potosí), CH (Chuquisaca), TJA (Tarija), BE (Beni), PD (Pando ),
  paterno varchar(50),
  materno varchar(50),
  nombre varchar(100),
...
create table Institución_Gobernante (
  id_intitución_gobernante int primary key,
  nombre varchar(100),
  sigla varchar(20),
  telefono varchar(20),
...
create table Incendio (
  id_incendio int primary key,
  tipo varchar(30), -- superficie, copa, subterráneo
  magnitud varchar(50),
  fecha_incio date,
  fecha_fin date, -- null si sigue vigente
  descripción varchar(100),
  origen varchar(30) -- natural, antrópico
);
GO

create table Organización (
  id_organización int primary key,
  telefono varchar(20), -- 71234567
  email varchar(100),
  ubicación varchar(150), -- departamento, municipio, ciudad, calle
  fecha_creación date,
  tipo varchar(30), -- voluntario, estatal
  nombre varchar(100)  -- (Organizaciones contra incendios volutarias o estatales, como Bomberos antofagasta, regimiento 3 de bomberos voluntarios, vecinos del distrito 6)
);
GO

create table Bombero (
  id_bombero int primary key,
  ci varchar(15), -- 1234567, sin expedido
  fecha_nac date,
  fecha_registro date,
  telefono varchar(20), -- 71234567
  ubicación_domicilio varchar(150), -- ciudad, calle ya que el ci tiene departamento
  email varchar(100),
  tipo_sangre varchar(10),
  telefono_emergencia varchar(20), -- 71234567
  nombre_emergencia varchar(100), --nombre completo (nombre paterno materno) de la persona a comunicarse en caso de emergencia
  fecha_inicio_actividades date,
  descripción_enfermedad_base varchar(100), -- si tiene un enfermedad base, si no poner 'ninguna'
...
create table Voluntario (
  ...
  descripción_enfermedad_base varchar(100), -- si tiene un enfermedad base, si no poner 'ninguna'
  tipo_sangre varchar(10),
  ubicacion_domicilio varchar(150), -- ciudad, calle ya que el ci tiene departamento
  foreign key (ci) references Persona(ci)
);
GO
...
create table Instructor (
  ...
  telefono varchar(20),
  email varchar(100),
  foreign key (id_organización) references Organización(id_organización),
  foreign key (ci) references Persona(ci)
);
GO
...
create table Brigada (
  ...
  id_organización int,
  nombre varchar(100), -- nombre de brigada contra incendios, ejmplo: Brigrada de volutnarios antogafata, brigrada de primeros auxilio san jose-- 1234567, sin expedido
  fecha_creacion date,
  ubicación_base_operaciones varchar(150), -- departamento, municipio, ciudad, calle
  foreign key (id_organización) references Organización(id_organización)
);
GO
...
create table Recurso (
  ...
  nombre varchar(100), -- recursos usados contra incendio, pueden ser nombre de uno y solo uno de los siguiente: vehiculos (areos, terrestres para la lucha contra inceidos), medicamentos, víveres, herramientas
  disponibilidad varchar(20), -- agotado, disponible
  descripcion varchar(100),
  marca varchar(50), -- empresa eque fabrico el producto
  estado varchar(20) -- bueno, malo, regular
);
GO

create table Registro_Brigada (
  id_registro_brigada int primary key,
  id_brigada int,
  id_voluntario int,
  id_bombero int,
  fecha date,
  tipo_registro varchar(20), -- inscripcion, salida
  rango varchar(20), -- ayudante, asistente, capitan, y demás rango que pertenecen a una brigada de bomberos
  foreign key (id_brigada) references Brigada(id_brigada),
  foreign key (id_voluntario) references Voluntario(id_voluntario),
  foreign key (id_bombero) references Bombero(id_bombero),
  -- para que  o bien id_voluntario es null o bien id_bombero es null pero no puden ser los dos al mismo tiempo null ni tampoco no null al mismo tieempo
  CONSTRAINT chk_voluntario_bombero CHECK (
    (id_voluntario IS NULL AND id_bombero IS NOT NULL) OR
    (id_voluntario IS NOT NULL AND id_bombero IS NULL)
  )
);
GO

create table Registro_Recurso (
  id_registro_recurso int primary key,
  id_brigada int,
  id_recurso int,
  tipo varchar(20), -- extracción, devolución
  fecha date,
  foreign key (id_brigada) references Brigada(id_brigada),
...
create table Medicamento (
  ...
  id_recurso int,
  tipo_uso varchar(20), -- oral, intravenosa
...
create table Viveres (
  id_vivere int primary key,
  id_recurso int,
  unidad_medida varchar(10),
  cantidad_unidad decimal(10,2),
  num_unidades int,
  categoria varchar(30), -- cereal, granos, carne...
  fecha_vencimiento date,
  foreign key (id_recurso) references Recurso(id_recurso)
);
GO
...
create table Dona (
  ...
  id_donante int,
  fecha date,
  metodo varchar(20), -- fisico, virtual
...
create table Afectado (
  id_afectado int primary key,
  ci varchar(15),
  id_area int,
  telefono varchar(20),
  ubicacion_domicilio varchar(150),
  email varchar(100),
  condición varchar(20), -- herido, bueno, crítico, muerte
  idioma varchar(50),
  foreign key (ci) references Persona(ci)
);
GO
...
create table Bien (
  ...
  id_afectado int,
  tipo varchar(20), -- mueble, inmueble
  magnitud_dano varchar(20), -- severo, leve
...
create table Reporte (
  ...
  id_incendio int,
  carácter varchar(20), -- urgente, prioritario, rutinario
  mensaje varchar(20),
...
create table Gobernacion (
  id_gobernación int primary key,
  id_intitución_gobernante int,
  id_reporte int,
  departamento varchar(50),
  num_provincias int,
  superficies decimal(10,2),
  foreign key (id_intitución_gobernante) references Institución_Gobernante(id_intitución_gobernante),
...
create table Alcaldia (
  ...
  num_consejales int,
  foreign key (id_intitución_gobernante) references Institución_Gobernante(id_intitución_gobernante),
  foreign key (id_gobernación) references Gobernacion(id_gobernación)
);
GO
...
create table Area_Afectada (
  ...
  id_municipio int,
  id_incendio int,
  tipo_vegetacion varchar(30), -- urbana, forestal
  descripción varchar(100),
  costo_daño decimal(10,2),
...
create table Acude (
  id_brigada int,
  id_area int,
  evaluacion_respuesta varchar(30), -- suficiente, insuficiente
  fecha_incursion date,
  fecha_retirada date,
  primary key (id_brigada, id_area),
  foreign key (id_brigada) references Brigada(id_brigada),
...
create table Alerta (
  ...
  primary key (id_organización, id_reporte),
  foreign key (id_organización) references Organización(id_organización),
  foreign key (id_reporte) references Reporte(id_reporte)
);
GO
